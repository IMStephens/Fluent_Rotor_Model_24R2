...
(make-new-rpvar 'my-number-rotor-zones-var 1 'integer)
(make-new-rpvar 'my-rotor '() 'list)
(make-new-rpvar 'my-surfaces '() 'thread-list)
(make-new-rpvar 'my-fusethread '() 'thread-list)
;(make-new-rpvar 'my-wall-surf '() 'thread-list)
(make-new-rpvar 'my-list-length-check 0 'integer)
...
; define->models->rotor model-> Rotor Inputs
		...
		(define (update-cb . args)
			(cx-set-integer-entry number-rotor-zones (rpgetvar 'my-number-rotor-zones-var))
...
;this fn.- apply-cb  will be called when the user clicks on "close" button
		...
		(define (apply-cb . args)
			(rpsetvar 'my-number-rotor-zones-var (cx-show-integer-entry number-rotor-zones ))
			(if (pair? (rpgetvar 'my-rotor))
				(rpsetvar 'my-list-length-check 1)
				(rpsetvar 'my-list-length-check 0) 
			)

; This portion of the code is added if the user reduces the number of rotor-zones - say from 4 to 2 - from here
			(if (< (rpgetvar 'my-number-rotor-zones-var) (length(rpgetvar 'my-rotor)) )
				(begin
					(let ( (diff) (rotor (rpgetvar 'my-rotor)) )
						(set! diff (- (length rotor) (rpgetvar 'my-number-rotor-zones-var) ) ) 
						(set! rotor (reverse (list-tail (reverse rotor) diff) ) )
						(rpsetvar 'my-rotor rotor)
					)
				)
			)
; Up to here
			
			(%udf-on-demand 'my_rotor_inputs::libudf) ; when the user clicks on OK button, the UDF will be executed 
		)

; this fn. - set-cb will be called when the user clicks on "Change/Create" button
; user should click on "Change/Create" button when he inputs some values for the 
; "active" rotor zone

; "trimming" and "geometry" are local lists inside set-cb function

		(define (set-cb . args)
			(let ((num) (col-p) (cyc-p) (dtc) (dxmc) (dymc) (fb) (mb) (cg-x) (cg-y) (cg-z) (general) (trimming) (geometry) (rotor (rpgetvar 'my-rotor)) )
				...
				(rpsetvar 'my-surfaces
					(map thread-name->id
						(cx-show-symbol-list-selections surfaces) ;This 'map' fnc applies function 'thread-name->id' to all elements in the surfaces list.
					)
				)
				
				(if (= (length(rpgetvar 'my-surfaces)) 0)
					(begin
						(format "\n\n Error: Please select a surface for Rotor Face Zone\n") 
						(rpsetvar 'my-surfaces (list 0) )
					)
				)
					
				(set! general (list-add general (rpgetvar 'my-surfaces) ) )
				...
				(if (cx-show-toggle-button trim)
					(begin
						...
						(if (cx-show-toggle-button for-b)
							(begin
								(set! fb 1)
								(rpsetvar 'my-fusethread (map thread-name->id (cx-show-symbol-list-selections fusethread) ) )
								(if (= (length(rpgetvar 'my-fusethread)) 0)
									(begin
										(format "\n\n Error: Please select a face thread for Fuselage\n")
										(rpsetvar 'my-fusethread (list 0) )
									)
								)
							)
							(begin
						...
						(if (cx-show-toggle-button mom-b)
							(begin
								(set! mb 1)
								(set! cg-x 1 )
								(set! cg-y 1 )
...
; then default values will be written 
		...
		(define (old-cb . args)
			(let ( (num) (dummy-rotor (rpgetvar 'my-rotor)) )
				(set! num (cx-show-integer-entry active-rotor-zone))
...
; Trim Update	
						...
						(if (= (list-ref tri 0) 1) 
							(begin
								...
								(cx-set-integer-entry update-frequency (list-ref tri 5))
								(cx-set-real-entry damping-factor (list-ref tri 6))
								(if (= (list-ref tri 1) 1) 
									(begin
										(cx-set-toggle-button collective-pitch #t)
										(cx-enable-item desired-thrust-coeff #t)
										(cx-set-real-entry desired-thrust-coeff (list-ref tri 7))
									)
									(begin
								...
								(if (= (list-ref tri 2) 1) 
									(begin
										...
										(cx-set-real-entry desired-x-mom-coeff (list-ref tri 8))
										(cx-set-real-entry desired-y-mom-coeff (list-ref tri 9))
										(if (= (list-ref tri 1) 1)
											(begin
												(cx-enable-item for-b #t)
												(if (= (list-ref tri 3) 1)
													(begin
														(cx-set-toggle-button for-b #t)
														(cx-enable-item mom-b #t)
														(if (= (list-ref tri 4) 1)
															(begin 
																(cx-set-toggle-button mom-b #t)
																(cx-enable-item fusethread #t)
																(cx-enable-item cg-prof-bb #t)
																(cx-set-toggle-button cg-prof1 #t)
																(cx-enable-item cgx #t)
																(cx-enable-item cgy #t)
																(cx-enable-item cgz #t)
																(fusefaceupdate-cb)
																(if (not (= (list-ref (list-ref tri 10) 0) 0) )
																	(cx-set-symbol-list-selections fusethread (map thread-id->name (list-ref tri 10) ))
																)
																(cx-set-real-entry cgx (list-ref tri 11))
																(cx-set-real-entry cgy (list-ref tri 12))
																(cx-set-real-entry cgz (list-ref tri 13))
									...
									(begin
										...
										(cx-set-toggle-button for-b #f)
										(cx-set-toggle-button mom-b #f)
										(cx-enable-item for-b #f)
										(cx-enable-item mom-b #f)
										(cx-enable-item fusethread #f)
										(cx-enable-item cg-prof-bb #f)
										(cx-enable-item cgx #f)
										(cx-enable-item cgy #f)
										(cx-enable-item cgz #f)
...
; end
						...
						(cx-enable-item mom-b #f)
						(cx-enable-item fusethread #f)
						(cx-set-real-entry cgx 0)
						(cx-enable-item cgx #f)
						(cx-set-real-entry cgy 0)
						(cx-enable-item cgy #f)
						(cx-set-real-entry cgz 0)
						(cx-enable-item cgz #f)
						(fusefaceupdate-cb)
						(cx-enable-item cg-prof-bb #f)
						(cx-set-toggle-button trim #f)
		...
		(define (desired-col-cb w label . args)
			(if (string=? label "Collective pitch")
				(begin
					...
					(if (and (cx-show-toggle-button cyclic-pitch) (cx-show-toggle-button collective-pitch))
						(cx-enable-item for-b #t)
						(cx-enable-item for-b #f)
					)
				)
...
; this fn. for-b-cb activates fuselage face thread list
; once force-balance toggle button is switched "on"
		(define (for-b-cb w label . args)
			(if (string=? label "Force balance")
				(begin
					(cx-enable-item fusethread (cx-show-toggle-button for-b))
					(cx-enable-item mom-b (cx-show-toggle-button for-b))
				)
			)
		)
		
; this fn. mom-b-cb activates fuselage CG position
; once moment-balance toggle button is switched "on"
		(define (mom-b-cb w label . args)
			(if (string=? label "Moment balance")
				(begin
					(cx-enable-item cg-prof-bb (cx-show-toggle-button mom-b))
					(cx-enable-item cgx (cx-show-toggle-button mom-b))
					(cx-enable-item cgy (cx-show-toggle-button mom-b))
					(cx-enable-item cgz (cx-show-toggle-button mom-b))
					(if (cx-show-toggle-button mom-b)
						(cx-set-toggle-button cg-prof1 #t)
					)
				)
...
; this fn. cg-prof-cb activates fuselage CG position
; once custom profile is selected
		(define (cg-prof-cb w label . args)
			(begin
				(cx-enable-item cgx (cx-show-toggle-button cg-prof1))
				(cx-enable-item cgy (cx-show-toggle-button cg-prof1))
				(cx-enable-item cgz (cx-show-toggle-button cg-prof1))
				(if (cx-show-toggle-button cg-prof2)
					(begin
						(cx-set-real-entry cgx 1)
						(cx-set-real-entry cgy 2)
						(cx-set-real-entry cgz 3)
					)
				)
			)
		)
...
; gets only interior surfaces under rotor face zone
		(define (surfaceupdate-cb . args)
			(cx-set-list-items surfaces (map thread-name (get-threads-of-type 'interior)))
		)
; gets only wall surfaces under fuselage face zone
		(define (fusefaceupdate-cb . args)
			(cx-set-list-items fusethread (map thread-name (get-threads-of-type 'wall)))
		)
...
;			----------
		...
		(lambda args
			...
			(if (not panel)
				(let ( (tabel) (frame) (frame1) (frame2) (frame3) (frame4) (frame5) )
					...
					(set! number-rotor-zones (cx-create-integer-entry box "Number of Rotor Zones" 
											'width 3
					...
					(set! active-rotor-zone (cx-create-integer-entry box4 "Active Rotor Zone" 
											'width 3
											'minimum 1 
											'label-position 'left-of
											'row 0 'col 0
...
;------------
					...
					(set! box0 (cx-create-table frame1 "" 'right-of 0 'below 0
											'table-options '("right-justify" "top-justify")
											))
					(set! number-of-blades (cx-create-integer-entry box0 "Number of Blades"
										'row 0 'col 0
										'minimum 1
										'label-position 'left-of
										'value 1))
					(set! rotor-radius (cx-create-real-entry box0 "Rotor Radius" 
										'row 1 'col 0
										'minimum 0.0
										'label-position 'left-of
										'value 0.0
										'units-quantity 'length))
					(set! rotor-speed (cx-create-real-entry box0 "Rotor Speed"
										'row 2 'col 0
										'label-position 'left-of
										'value 0.0
										'units-quantity 'angular-velocity))
					(set! tip-effect (cx-create-real-entry box0 "Tip Effect (%)"
										'row 3 'col 0
										'minimum 0.0
										'label-position 'left-of
										'value 96.0) )
					(set! box2 (cx-create-table frame1 "Rotor Disk Origin" 
										'right-of 0
										'table-options '("right-justify" "top-justify")
										'below tip-effect))
					(set! rotor-origin-x (cx-create-real-entry box2 "X" 
										'row 0 'col 0
										'units-quantity 'length 
										'width 5
										'label-position 'left-of
										'value 0.0 ) )
					(set! rotor-origin-y (cx-create-real-entry box2 "Y" 
										'units-quantity 'length 
										'width 5
										'label-position 'left-of
										'value 0.0 
										'row 1 'col 0) )
					(set! rotor-origin-z (cx-create-real-entry box2 "Z" 
										'units-quantity 'length 
										'width 5
										'label-position 'left-of
										'value 0.0 
					...
					(set! box3 (cx-create-table frame1 "Rotor Face Zone" 
										'right-of box2 'below tip-effect
										'table-options '("right-justify" "top-justify")) )
					(set! surfaces (cx-create-symbol-list box3 "Surfaces"
										'visible-lines 4 'width 18 
					...
					(set! rotor-disk-pitch-angle (cx-create-real-entry box5 "Pitch Angle" 
										'units-quantity 'angle
										'row 0 'col 0 'width 5
										'label-position 'left-of ) )									
					(set! rotor-disk-bank-angle (cx-create-real-entry box5 "Bank Angle" 
										'units-quantity 'angle
										'row 1 'col 0 'width 5
										'label-position 'left-of) )
					...
					(set! box6 (cx-create-table frame2 "Blade Pitch" 'below box5 'table-options
											'("right-justify" "top-justify") ) )
					(set! blade-pitch-collective (cx-create-real-entry box6 "Collective" 
										'units-quantity 'angle
										'row 0 'col 0 'width 5
										'label-position 'left-of ) )
					(set! blade-pitch-cyclic-sin (cx-create-real-entry box6 "Cyclic Sin" 
										'units-quantity 'angle
										'row 1 'col 0 'width 5
										'label-position 'left-of ) )
					(set! blade-pitch-cyclic-cos (cx-create-real-entry box6 "Cyclic Cos" 
										'units-quantity 'angle
										'row 2 'col 0 'width 5
										'label-position 'left-of ) )									
					(set! box7 (cx-create-table frame2 "Blade Flapping" 'below box6 'table-options
											'("right-justify" "top-justify") ) )
					(set! blade-flap-cone (cx-create-real-entry box7 "Cone" 
										'units-quantity 'angle
										'row 0 'col 0 'width 5
										'label-position 'left-of ) )
					(set! blade-flap-cyclic-sin (cx-create-real-entry box7 "Cyclic Sin" 
										'units-quantity 'angle
										'row 1 'col 0 'width 5
										'label-position 'left-of ) )
					(set! blade-flap-cyclic-cos (cx-create-real-entry box7 "Cyclic Cos" 
										'units-quantity 'angle
										'row 2 'col 0 'width 5
										'label-position 'left-of ) )
...
;-------------
					...
					(set! frame3 (cx-create-frame trimm "" 'right-of 0 'below 0) )
					(set! box8 (cx-create-table frame3 "" 'below 0 'right-of 0
										'table-options '("right-justify" "top-justify") ))
					...
					(set! cyclic-pitch (cx-create-toggle-button box8 "Cyclic pitch"
										...
										'state #f 'row 0 'col 1) )
					(cx-set-toggle-button cyclic-pitch #f)
					(set! for-b (cx-create-toggle-button box8 "Force balance"
										'activate-callback for-b-cb
										'state #f 'row 0 'col 2) )
					(cx-enable-item for-b #f)
					
					(set! mom-b (cx-create-toggle-button box8 "Moment balance"
										'activate-callback mom-b-cb
										'state #f 'row 0 'col 3) )
					(cx-enable-item mom-b #f)
					
			
					(set! box9 (cx-create-table frame3 "" 'below box8 'right-of 0 
										'table-options '("right-justify" "top-justify"))) 
					(set! update-frequency (cx-create-integer-entry box9 "Update Frequency" 
										'width 3
										'minimum 1 'label-position 'left-of
										'value 10 'row 0 'col 0) )
					(set! damping-factor (cx-create-real-entry box9 "Damping Factor" 
										'width 5
										'minimum 0.0 'label-position 'left-of
										'value 0.1 'row 1 'col 0))
					(set! desired-thrust-coeff (cx-create-real-entry box9 "Desired thrust coefficient" 
										'width 5
										'minimum 0.0 'label-position 'left-of
										'row 2 'col 0 ))
					(cx-enable-item desired-thrust-coeff #f) 
			
					(set! desired-x-mom-coeff (cx-create-real-entry box9 "Desired x-moment coefficient" 
										'width 5
										'minimum 0.0 'label-position 'left-of
										'row 3 'col 0 ))
					(cx-enable-item desired-x-mom-coeff #f) 
			
					(set! desired-y-mom-coeff (cx-create-real-entry box9 "Desired y-moment coefficient" 
										'width 5
										'minimum 0.0 'label-position 'left-of
										'row 4 'col 0 ))
					(cx-enable-item desired-y-mom-coeff #f) 
					...
					(set! frame4 (cx-create-frame trimm "" 'right-of frame3 'below 0) )
					
					(set! box16 (cx-create-table frame4 "Fuselage Face Thread" 'right-of 0 'below 0
										'table-options '("left-justify" "top-justify")) )
					(set! fusethread (cx-create-symbol-list box16 "Face Threads"
										'visible-lines 4 'width 14
										'multiple-selections #f
										'row 0 'col 0))
					(cx-set-list-items fusethread (map thread-name (get-threads-of-type 'wall)))
					
					
					(set! frame5 (cx-create-frame trimm "" 'right-of frame4 'below 0) )
					
					(set! box17 (cx-create-table frame5 "Define CG Position" 
										'right-of 0 'below 0
										'table-options '("left-justify" "top-justify")) )
					(set! cg-prof-bb (cx-create-button-box box17 "Existing Profiles" 
										'radio-mode #t
										'row 0 'col 0))			
					(cx-enable-item cg-prof-bb #f)					
					(set! cg-prof1 (cx-create-toggle-button cg-prof-bb "Custom" 
										'activate-callback cg-prof-cb))
					(cx-set-toggle-button cg-prof1 #f)	
					
					(set! cg-prof2 (cx-create-toggle-button cg-prof-bb "NATO-GR" 
										'activate-callback cg-prof-cb))
					(cx-set-toggle-button cg-prof2 #f)		
					
					(set! cgx (cx-create-real-entry box17 "Desired CG X" 
										'width 5
										'minimum 0.0 'label-position 'left-of
										'row 1 'col 0 ))
					(cx-enable-item cgx #f)
					
					(set! cgy (cx-create-real-entry box17 "Desired CG Y" 
										'width 5
										'minimum 0.0 'label-position 'left-of
										'row 2 'col 0 ))
					(cx-enable-item cgy #f)
					
					(set! cgz (cx-create-real-entry box17 "Desired CG Z" 
											'width 5
											'minimum 0.0 'label-position 'left-of
											'row 3 'col 0 ))
					(cx-enable-item cgz #f)
...
(in-package models-package
    (define heli-model (instance generic-model))
    (in-package heli-model
        (define-method (model-name) "Helicopter Model")
        (define-method (get-model-setting) 
			(cond (heli-model? "On") 
				(else "Off"))
		)
        (define-method (available?) 
			(and heli-model-available? 
				(^send available?))
		)
...
(define heli-model? #f)
(define heli-model-available? #t)

(register-model-in-models-taskpage 'heli-model)
