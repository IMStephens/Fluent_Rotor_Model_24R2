...
real sigma_az_old[max_rotor_zones] = {0.0}; /* Previous value of sigma_az. */
real sigma_vert_old[max_rotor_zones] = {0.0}; /* Previous value of sigma_vert. */
real gauss_denom_old[max_rotor_zones] = {0.0}; /* Previous value of Gaussian denominator. */
...
/*---------------------------------------------------------------------------*/
  void my_force_blade_to_xyz(int i, cell_t cc, Thread *tc, int count,
    ...
    real *sigma_az, real *sigma_vert, real *gauss_denom, int *gauss_init,
...
/*---------------------------------------------------------------------------*/
  {
    ...
    if(isunst == 1){
      ...
      /* If Gaussian values haven't yet been set, give them initial values. */
      if((sigma_az_old[i]) == 0.0 && (sigma_vert_old[i]) == 0.0 && 
        (gauss_denom_old[i]) == 0.0){

        /* The azimuthal distance is normalised so avoid linking sigma to 
            rotor chord. */
        sigma_az_old[i] = 0.08;
        sigma_vert_old[i] = sigma_az_old[i] / 2.0;
        gauss_denom_old[i] = 0.75;
        *gauss_init = 1;
      }
      else
        *gauss_init = 0;
      ...
      (*sigma_az) = sigma_az_old[i];
      (*sigma_vert) = sigma_vert_old[i];
      (*gauss_denom) = gauss_denom_old[i];
      ...
      for(blade_psi_init = 0; blade_psi_init < (2.0 * M_PI); blade_psi_init = 
        ...
        *deltabeta = fabs(beta_pb - beta); 
        ...
        if (psi_pb > 2.0 * M_PI + blade_psi_curr - bsa/2.0){
          (*deltapsi) = 2.0 * M_PI + blade_psi_curr - psi_pb;
        }
        else if(2.0 * M_PI + psi_pb < blade_psi_curr + bsa/2.0){
          (*deltapsi) = 2.0 * M_PI + psi_pb - blade_psi_curr;
        }
        ...
        if((*deltapsi) < bsa/2.0){
          ...
          *az_dist = fabs(r_pb * (*deltapsi));
          /* Similar to above for vertical distance. */
          *vert_dist = fabs(r_pb * (*deltabeta));
...
/*---------------------------------------------------------------------------*/
  void my_gauss_opt(int i, real sum_sample_vol, real sum_gwf, int gauss_active, 
    ...
    real *gauss_denom, real *sigma_vert, real *sigma_az, int gauss_init,
...
/*---------------------------------------------------------------------------*/
  {
    ...
    if(gauss_routine){
      ...
      if(gauss_init == 0){
        ...
        delta_sigma_vert = (*sigma_vert) - sigma_vert_old[i];
        delta_gauss_denom = (*gauss_denom) - gauss_denom_old[i];*/
        delta_sigma_az = (*sigma_az) - sigma_az_old[i];

        /* Calculate new multiplication factors. */
        if(gauss_init == 0){
          Message("  %d sigma_az = %f, sigma_az_old = %f, delta_sigma_az = %f \n", 
            myid, (*sigma_az), sigma_az_old[i], delta_sigma_az);

          /* Find gradient of delta cells to delta sigmas and delta denom. 
          delta_cabr_sigvert = delta_c_ab_r / delta_sigma_vert;*/
          delta_cahr_sigaz = delta_c_ah_bh_r / delta_sigma_az;
          /*Message("  %d delta_cahr_sigaz = %f.", myid, delta_cahr_sigaz);*/

          /* Calculate new sigma and denom values. 
          sigma_vert_new = (*sigma_vert) + trdf[i] * (-total_cells_above_rotor / 
            delta_cabr_sigvert);*/
          sigma_az_new = (*sigma_az) + trdf[i] * (-global_cells_ahead_behind_rotor / 
            delta_cahr_sigaz );
          if(delta_cahr_sigaz == 0){
            sigma_az_new = (*sigma_az);
          }

          /* Set new sigma and denom values. 
          (*sigma_vert) = sigma_vert_new;*/
          (*sigma_az) = sigma_az_new;
          /*Message(" %d sigma_az_new = %f", myid, *sigma_az);*/
        }
      }
      else{
        (*saz_fac) = 1.0;
        ...
        /*if (total_cells_above_rotor > 500)
          (*sigma_vert) *= 0.95;
        else
          (*sigma_vert) *= 1.05;*/

        if (global_cells_ahead_behind_rotor > 500)
          (*saz_fac) = 0.95;

        /*if (global_sum_gwf/global_sum_gauss_active > 1.1)
          (*gauss_denom) *= 1.05;
          
        if (global_sum_gwf/global_sum_gauss_active < 0.90){
          (*gauss_denom) *= 0.95;
        }*/
      }
      /* Save sigma and denom values. 
      sigma_vert_old[i] = (*sigma_vert);
      gauss_denom_old[i] = (*gauss_denom);*/
      sigma_az_old[i] *= (*saz_fac);
...

{
  Message0("!! Info: Ensure you have pre-allocated at least %d UDM locations in the GUI/TUI !!", NUM_UDM+2);
	...
	Set_User_Memory_Name(6, "Mach Number at Section");
...
/*---------------------------------------------------------------------------*/
{
    ...
    int gauss_init[max_rotor_zones]; /* Flag to indicate if Gaussian 
      parameters have been initialized for each rotor. */
    ...
    while (i < nrtz) {
      ...
      begin_c_loop_int(cc,tc)
        ...
        my_force_blade_to_xyz(i, cc, tc, count, 
          ...
          &sigma_az, &sigma_vert, &gauss_denom, &gauss_init[i],
      ...
      if(isunst)
        my_gauss_opt(i, sum_sample_vol, sum_gwf, gauss_active, 
          ...
          &gauss_denom, &sigma_vert, &sigma_az, gauss_init[i],
